...
import {course_box, image_wrapper, img, course_info, tags, title, price, teacher} from './course.module.scss'
          ...
          <div className={course_box} key={item._id}>
            <Link href={`/courses/${item._id}`}>
              ...
              <div className={course_info}>
                ...
                <p className={price}>$ {item.price} <span className={teacher}>| {item.teacherID?.fullname}</span></p>
              </div>
            </Link>
          </div>
        )})
